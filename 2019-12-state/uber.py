...
state = st.State()
if not state:
    state.data = pd.read_csv(DATA_URL, nrows=100000)
    lowercase = lambda x: str(x).lower()
    state.data.rename(lowercase, axis="columns", inplace=True)
    state.data[DATE_TIME] = pd.to_datetime(state.data[DATE_TIME])
